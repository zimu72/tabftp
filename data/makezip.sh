...
targetdir="win32zip/TabFTP-$PACKAGE_VERSION"
...
if [ "${FILEZILLA_LINGUAS}" != "" ]; then
  ...
  for i in $FILEZILLA_LINGUAS; do
    ...
    cp "$top_builddir/locales/$i.mo" "$targetdir/locales/$i/tabftp.mo" || exit 1
  done
...
cp -r "$install_prefix/share/tabftp/resources" "$targetdir/resources" || exit 1
...
cd win32zip
zip -r -9 ../TabFTP.zip "TabFTP-$PACKAGE_VERSION" || exit 1
cd ..
