...
CDBusHandler::CDBusHandler()
{
	...
	wxString sessionBusAddress;
	wxString starterBusType;
	bool const haveSessionBusAddress = wxGetEnv(_T("DBUS_SESSION_BUS_ADDRESS"), &sessionBusAddress) && !sessionBusAddress.empty();
	bool const startedForSessionBus = wxGetEnv(_T("DBUS_STARTER_BUS_TYPE"), &starterBusType) && starterBusType == _T("session");

	if (!haveSessionBusAddress && !startedForSessionBus) {
		if (m_debug) {
			printf("wxD-Bus: No session bus address, skipping D-Bus initialization\n");
		}
		m_pConnection = 0;
		return;
	}
