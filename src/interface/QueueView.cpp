...
bool CQueueView::QueueFiles(const bool queueOnly, Site const& site, CLocalRecursiveOperation::listing const& listing)
{
	CServerItem* pServerItem = CreateServerItem(site);
...
bool CQueueView::CanStartTransfer(CServerItem const & server_item, t_EngineData *&pEngineData)
{
	...
	if (!site.connection_limit_) {
		return true;
	}

	unsigned int active_count = static_cast<unsigned int>(server_item.m_activeCount);

	CState* browsingStateOnSameServer = 0;
	const std::vector<CState*> *pStates = CContextManager::Get()->GetAllStates();
	for (auto pState : *pStates) {
		Site const& browsingSite = pState->GetSite();
		if (!browsingSite) {
			continue;
		}
	...
	if (active_count < site.connection_limit_) {
		return true;
	}

	// Max count has been reached

	pEngineData = GetIdleEngine(site, true);
	if (pEngineData) {
		...
		if (pEngineData->lastSite == site && pEngineData->pEngine->IsConnected()) {
			return true;
		}
	}
	...
	// connection_limit_ is limited to 1, only connection to server is held by the browsing connection
...
bool CQueueView::TryStartNextTransfer()
{
	...
	Site const oldSite = pEngineData->lastSite;
	pEngineData->lastSite = bestMatch.serverItem->GetSite();
	...
	if (pEngineData->state != t_EngineData::waitprimary) {
		...
		else if (oldSite != bestMatch.serverItem->GetSite()) {
	...
	if (data.pItem) {
		...
		if (data.pItem->GetType() == QueueItemType::File) {
			...
			if (pFileItem->m_edit != CEditHandler::none && reason != ResetReason::retry && reason != ResetReason::reset) {
				CEditHandler* pEditHandler = m_pMainFrame->GetEditHandler();
				if (pEditHandler) {
					if (pFileItem->m_edit == CEditHandler::remote) {
						pEditHandler->FinishTransfer(reason == ResetReason::success, pFileItem->GetRemoteFile(), pFileItem->GetRemotePath(), pServerItem->GetSite());
					}
					else {
			...
			if (reason == ResetReason::success) {
				pFileItem->ExecutePostTransferAction(true);
			} else if (reason == ResetReason::failure) {
				pFileItem->ExecutePostTransferAction(false);
			}
		...
		if (reason == ResetReason::reset) {
			...
			if (data.pItem->GetType() == QueueItemType::File || data.pItem->GetType() == QueueItemType::Folder) {
				Site const site = ((CServerItem*)data.pItem->GetTopLevelItem())->GetSite();

				RemoveItem(data.pItem, false);

				CQueueViewFailed* pQueueViewFailed = m_pQueue->GetQueueView_Failed();
				CServerItem* pNewServerItem = pQueueViewFailed->CreateServerItem(site);
			...
			if (data.pItem->GetType() == QueueItemType::File || data.pItem->GetType() == QueueItemType::Folder) {
				...
				if (pQueueViewSuccessful->AutoClear()) {
					...
					static_cast<CFileItem&>(*data.pItem).clear_persistent_state();
					data.pItem->UpdateTime();
					data.pItem->SetParent(pNewServerItem);
...
void CQueueView::SendNextCommand(t_EngineData& engineData)
{
	for (;;) {
		...
		if (engineData.state == t_EngineData::connect) {
			...
			int res = engineData.pEngine->Execute(CConnectCommand(engineData.lastSite.server, engineData.lastSite.Handle(), engineData.lastSite.credentials, false));
...
void CQueueView::LoadQueue()
{
	...
	if (!m_queue_storage.BeginTransaction()) {
		...
		Site site;
		int64_t const first_id = m_queue_storage.GetServer(site, true);
		auto id = first_id;
		for (; id > 0; id = m_queue_storage.GetServer(site, false)) {
			m_insertionStart = -1;
			m_insertionCount = 0;
			CServerItem *pServerItem = CreateServerItem(site);
...
void CQueueView::OnContextMenu(wxContextMenuEvent&)
{
    ...
    menuAfter->Append(XRCID("ID_ACTIONAFTER_CLOSE"), _("&Close TabFTP"), wxString(), wxITEM_CHECK);
    ...
    menuAfter->Append(XRCID("ID_ACTIONAFTER_CLOSE_ONCE"), _("&Close TabFTP once"), wxString(), wxITEM_CHECK);
...
t_EngineData* CQueueView::GetIdleEngine(Site const& site, bool allowTransient)
{
	wxASSERT(!allowTransient || site);
	...
	for (unsigned int i = 0; i < m_engineData.size(); ++i) {
		...
		if (m_engineData[i]->pEngine->IsConnected() && m_engineData[i]->lastSite == site) {
			return m_engineData[i];
		}
...
void CQueueView::TryRefreshListings()
{
	...
	for (std::vector<CState*>::const_iterator iter = pStates->begin(); iter != pStates->end(); ++iter) {
		...
		for (i = 0; i < m_engineData.size(); ++i) {
			...
			if (m_engineData[i]->lastSite != site) {
				continue;
			}
		...
		t_EngineData* pEngineData = GetIdleEngine(site);
		if (!pEngineData) {
			continue;
		}

		if (!pEngineData->pEngine->IsConnected() || pEngineData->lastSite != site) {
			continue;
		}
	...
	for (auto & pData : m_engineData) {
		if (pData->m_idleDisconnectTimer && !pData->m_idleDisconnectTimer->IsRunning()) {
			...
			if (pData->pEngine->IsConnected()) {
				(void)pData->pEngine->Execute(CDisconnectCommand());
			}
		}
	}
...
void CQueueView::OnExclusiveEngineRequestGranted(unsigned int requestId)
{
	...
	Site const& currentSite = pState->GetSite();
...
bool CQueueView::IsOtherEngineConnected(t_EngineData* pEngineData)
{
	for (auto const* current : m_engineData) {
		...
		if (current->lastSite != pEngineData->lastSite) {
			continue;
		}
...
void CQueueView::OnOptionsChanged(watched_options const&)
{
	if (m_activeMode) {
		AdvanceQueue();
	}
...
void CQueueView::QueueProxyTransfer(const Site& sourceSite, const CServerPath& sourcePath, const std::wstring& fileName, const Site& targetSite, const CServerPath& targetPath)
{
	CServerItem* pServerItem = CreateServerItem(sourceSite);
	if (!pServerItem) return;

	wxString tempDir = wxStandardPaths::Get().GetTempDir() + wxFileName::GetPathSeparator() + "TabFTP_Proxy";
	if (!wxFileName::DirExists(tempDir)) wxFileName::Mkdir(tempDir, wxS_DIR_DEFAULT, wxPATH_MKDIR_FULL);

	CLocalPath localTempPath(tempDir.ToStdWstring());

	// Queue Download
	CFileItem* pDownloadItem = new CFileItem(pServerItem, transfer_flags::download, fileName, fileName, localTempPath, sourcePath, -1, L"");

	// Set Action to queue upload upon success
	pDownloadItem->SetPostTransferAction([this, targetSite, targetPath, fileName, localTempPath](bool success) {
		if (success) {
			// Queue Upload
			CServerItem* pTargetServerItem = CreateServerItem(targetSite);
			if (pTargetServerItem) {
				CFileItem* pUploadItem = new CFileItem(pTargetServerItem, transfer_flags{}, fileName, fileName, localTempPath, targetPath, -1, L"");

				// Delete temp file after upload
				pUploadItem->SetPostTransferAction([localTempPath, fileName](bool) {
					wxString fullPath = localTempPath.GetPath() + fileName;
					if (wxFileExists(fullPath)) wxRemoveFile(fullPath);
				});

				pUploadItem->SetPriorityRaw(QueuePriority::highest);
				InsertItem(pTargetServerItem, pUploadItem);
				QueueFile_Finish(true);
			}
		} else {
			// Cleanup temp if download failed
			wxString fullPath = localTempPath.GetPath() + fileName;
			if (wxFileExists(fullPath)) wxRemoveFile(fullPath);
		}
	});

	pDownloadItem->SetPriorityRaw(QueuePriority::highest);
	InsertItem(pServerItem, pDownloadItem);
	QueueFile_Finish(true);
}
