	...
	if (code == error || (reply != L"ok" && reply != L"convertible")) {
		if (!silent) {
			...
			if (reply == _T("incompatible")) {
				msg.Printf(_("The file '%s' contains an SSH1 key. The SSH1 protocol has been deprecated, TabFTP only supports SSH2 keys."), keyFile);
			}
			else {
...
bool CFZPuttyGenInterface::LoadKeyFile(std::wstring& keyFile, bool silent, std::wstring& comment, std::wstring& data)
{
	...
	if (needs_conversion) {
		...
		wxString msg = wxString::Format(_("The file '%s' is not in a format supported by TabFTP.\nWould you like to convert it into a supported format?"), keyFile);
...
bool CFZPuttyGenInterface::LoadProcess(bool silent)
{
	...
	if (pos == std::wstring::npos) {
		if (!silent) {
			wxMessageBoxEx(_("fzputtygen could not be started.\nPlease make sure this executable exists in the same directory as the main TabFTP executable."), _("Error starting program"), wxICON_EXCLAMATION);
		}
		return false;
	}
