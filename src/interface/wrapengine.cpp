...
struct locale_file final
{
	std::wstring dir;
	std::wstring file;
};

static locale_file GetLocaleFile(std::wstring const& localesDir, std::wstring name)
{
	if (wxFileName::FileExists(localesDir + name + L"/tabftp.mo")) {
		return { name, L"tabftp.mo" };
	}
	if (wxFileName::FileExists(localesDir + name + L"/LC_MESSAGES/tabftp.mo")) {
		return { name + L"/LC_MESSAGES", L"tabftp.mo" };
	}
	if (wxFileName::FileExists(localesDir + name + L"/filezilla.mo")) {
		return { name, L"filezilla.mo" };
	}
	if (wxFileName::FileExists(localesDir + name + L"/LC_MESSAGES/filezilla.mo")) {
		return { name + L"/LC_MESSAGES", L"filezilla.mo" };
	}
...
bool CWrapEngine::LoadCache()
{
	...
	if (language != L"default") {
		...
		locale_file localeFile = GetLocaleFile(localesDir.GetPath(), language);
		if (!localeFile.dir.empty()) {
			fz::datetime const date = fz::local_filesys::get_modification_time(fz::to_native(localesDir.GetPath() + localeFile.dir + L"/" + localeFile.file));
			std::wstring const ticks = std::to_wstring(date.get_time_t());
