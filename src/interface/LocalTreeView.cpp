...
public:
	...
	virtual wxDragResult OnData(wxCoord x, wxCoord y, wxDragResult def)
	{
		...
		if (format == m_pFileDataObject->GetFormat()) {
			...
			// For global local tree view, get the site from the current active remote tab
			Site currentSite;
			CState* pDownloadState = nullptr;
			
			if (m_pLocalTreeView->m_pContextControl) {
				auto *controls = m_pLocalTreeView->m_pContextControl->GetControlsFromTabIndex(
					m_pLocalTreeView->m_pContextControl->GetCurrentTab());
			...
			// Fallback to local tree view's state
			auto & state = m_pLocalTreeView->m_state;
			if (!currentSite && state.GetSite()) {
				currentSite = state.GetSite();
				pDownloadState = &state;
			}
			...
			if (!pDownloadState || !pDownloadState->DownloadDroppedFiles(obj, path)) {
				return wxDragNone;
			}
		}
...
EVT_MENU(XRCID("ID_UPLOAD_TO_ALL"), CLocalTreeView::OnMenuUploadToAll)
...
void CLocalTreeView::OnContextMenu(wxTreeEvent& event)
{
	...
	bool remoteConnected = m_state.IsRemoteConnected() && !m_state.GetRemotePath().empty();
	if (!remoteConnected && m_pContextControl) {
		...
		for (size_t i = 0; i < m_pContextControl->GetTabCount(); ++i) {
			...
			if (controls && controls->pState && controls->pState->IsRemoteConnected() && !controls->pState->GetRemotePath().empty()) {
	...
	if (m_pContextControl && remoteConnected) {
		...
		if (connectedCount > 1) {
			auto item = new wxMenuItem(&menu, XRCID("ID_UPLOAD_TO_ALL"), _("Upload to &all remote sites"), _("Upload selected directory to all connected remote sites"));
			item->SetBitmap(MakeBmpBundle(wxArtProvider::GetBitmap(_T("ART_UPLOAD"), wxART_MENU)));
			menu.Append(item);
		}
	}
...
void CLocalTreeView::OnMenuUpload(wxCommandEvent& event, CState* pState)
{
	if (!pState) {
		wxBell();
		return;
...
void CLocalTreeView::OnMenuUploadToAll(wxCommandEvent& event)
{
	if (!m_pContextControl) {
		wxBell();
		return;
