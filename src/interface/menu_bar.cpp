	...
	, options_(options)
{
	...
	file->Append(XRCID("wxID_EXIT"), _("E&xit"), _("Close TabFTP"))->SetAccel(&accel);
	...
	edit->Append(XRCID("wxID_PREFERENCES"), _("&Settings..."), _("Open the settings dialog of TabFTP"));
#ifdef FZ_MAC
	edit->Append(XRCID("ID_MENU_EDIT_SANDBOX_DIRECTORIES"), _("&Directory access permissions..."), _("Open the directory access permissions dialog to configure the local directories TabFTP has access to."));
#endif
	...
	if (options_.get_int(OPTION_DEFAULT_DISABLEUPDATECHECK) == 0) {
		help->Append(XRCID("ID_CHECKFORUPDATES"), _("Check for &updates..."), _("Check for newer versions of TabFTP"));
