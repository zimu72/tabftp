...
CStatusLineCtrl::CStatusLineCtrl(CQueueView* pParent, COptionsBase& options, t_EngineData const* const pEngineData, wxRect const& initialPosition)
	: m_pParent(pParent)
	, options_(options)
...
void CStatusLineCtrl::InitFieldOffsets()
{
	...
	// @translator: This is a date/time formatting specifier. See https://docs.wxwidgets.org/stable/classwx_time_span.html
...
void CStatusLineCtrl::OnPaint(wxPaintEvent&)
{
	...
	if (status_.empty()) {
		...
		if (options_.get_int(OPTION_SPEED_DISPLAY)) {
		...
		const wxString bytestr = SizeFormatter::Format(status_.currentOffset, true, SizeFormat::bytes, options_.get_bool(OPTION_SIZE_USETHOUSANDSEP), 0);
		if (elapsed_milli_seconds >= 1000 && rate > -1) {
			...
			const wxString ratestr = SizeFormatter::Format(rate, true,
														 format,
...
void CStatusLineCtrl::ClearTransferStatus()
{
	...
	case t_EngineData::connect:
		m_statusText = wxString::Format(_("Connecting to %s"), m_pEngineData->lastSite.Format(ServerFormat::with_user_and_optional_port));
		break;
	default:
