...
static wxIcon GetCustomAppIcon(int size)
{
	wxMemoryInputStream is(appicon_png, appicon_png_len);
...
#ifdef __WXGTK__
static GdkPixbuf* CreateGtkAppIconPixbuf(int size)
{
	...
	if (!gdk_pixbuf_loader_write(loader, reinterpret_cast<guchar const*>(appicon_png), appicon_png_len, &error)) {
		if (error) {
			g_error_free(error);
		}
...
static GtkWidget* FindGtkMenuBar(GtkWidget* widget)
{
	if (!widget) {
		return nullptr;
	}
	...
	for (GList* l = children; l; l = l->next) {
		...
		GtkWidget* found = FindGtkMenuBar(child);
		if (found) {
...
static void ApplyGtkPaleBlueChrome(GtkWidget* headerbar, GtkWidget* menubar)
{
	...
	char const* css =
		"headerbar, headerbar.titlebar, .titlebar, .titlebar headerbar {"
	...
	virtual void OnStateChange(CState* pState, t_statechange_notifications notification, std::wstring const&, const void*) override
	{
		if (notification == STATECHANGE_CHANGEDCONTEXT) {
			...
			if (!pState || !pState->GetSite()) {
				m_pMainFrame->SetTitle(_T("TabFTP"));
			}
			else {
				m_pMainFrame->SetTitle(pState->GetTitle() + _T(" - TabFTP"));
			}
...
void CMainFrame::SetTitle(wxString const& title)
{
	wxFrame::SetTitle(title);
#ifdef __WXGTK__
	if (m_gtk_headerbar_title_label) {
		gtk_label_set_text(GTK_LABEL(m_gtk_headerbar_title_label), title.utf8_str());
	}
#endif
}
	...
	, m_comparisonToggleAcceleratorId(wxNewId())
{
	...
	Create(NULL, -1, _T("TabFTP"), wxDefaultPosition, initial_size);
	SetSizeHints(700, 500);
    SetBackgroundColour(wxColour(230, 240, 255)); // Pale Blue Background
	...
	::SetWindowText((HWND)pChild->GetHandle(), _T("TabFTP process identificator 3919DB0A-082D-4560-8E2F-381A35969FB4"));
#endif
	...
	wxIconBundle bundle;
	{
		...
		wxImage img(is, wxBITMAP_TYPE_PNG);
		if (img.IsOk()) {
			for (int const size : {16, 24, 32, 48, 64, 128, 256}) {
				wxImage scaled = img;
				if (scaled.GetWidth() != size || scaled.GetHeight() != size) {
					scaled.Rescale(size, size, wxIMAGE_QUALITY_HIGH);
				}
				wxIcon icon;
	...
	if (bundle.GetIconCount()) {
		SetIcons(bundle);
		wxIcon icon = GetCustomAppIcon(48);
		if (icon.IsOk()) {
			SetIcon(icon);
		}
	}
	else {
		auto const themeBundle = CThemeProvider::GetIconBundle(_T("ART_TABFTP"));
		SetIcons(themeBundle);
		SetIcon(CThemeProvider::GetIcon(_T("ART_TABFTP")));
	}
#endif

#ifdef __WXGTK__
	{
		GtkWidget* window = GTK_WIDGET(GetHandle());
		if (window) {
			...
			GtkWidget* title_label = gtk_label_new(GetTitle().utf8_str());
			gtk_widget_set_halign(title_label, GTK_ALIGN_START);
			gtk_widget_set_valign(title_label, GTK_ALIGN_CENTER);
			gtk_label_set_xalign(GTK_LABEL(title_label), 0.0f);
			gtk_box_pack_start(GTK_BOX(title_box), title_label, TRUE, TRUE, 0);
			...
			m_gtk_headerbar = header;
			m_gtk_headerbar_title_label = title_label;

			GdkPixbuf* pixbuf = CreateGtkAppIconPixbuf(48);
			if (pixbuf) {
...
#ifdef __WXGTK__
	{
		...
		if (window) {
			...
			menubar = FindGtkMenuBar(child);
		}
		ApplyGtkPaleBlueChrome(m_gtk_headerbar, menubar);
	}
#endif
	CreateMainToolBar();
	{
		wxColour chrome(230, 240, 255);
		if (m_pToolBar) {
			m_pToolBar->SetBackgroundColour(chrome);
		}
		if (m_pStatusBar) {
			m_pStatusBar->SetBackgroundColour(chrome);
		}
	}
...
void CMainFrame::OnMenuHandler(wxCommandEvent &event)
{
	...
	// Debug menu
		...
		if (showHidden) {
			...
			dlg.AddText(_("Keep in mind that not all servers support this feature and may return incorrect listings if this option is enabled. Although TabFTP performs some tests to check if the server supports this feature, the test may fail."));
			 ...
			 id == XRCID("ID_MENU_HELP_BUGREPORT"))
	{
		...
		if (id == XRCID("ID_MENU_HELP_GETTINGHELP")) {
			url = _T("https://github.com/zimu72/tabftp");
		}
		else {
			url = _T("https://github.com/zimu72/tabftp/issues");
		}
		...
		if (m_pContextControl && m_pContextControl->GetGlobalLocalListViewPanel()) {
			auto* pStatusBar = m_pContextControl->GetGlobalLocalListViewPanel()->GetStatusBar();
			if (pStatusBar) {
				pStatusBar->Show(show);
				wxSizeEvent evt;
				m_pContextControl->GetGlobalLocalListViewPanel()->ProcessWindowEvent(evt);
			}
		}
...
void CMainFrame::OnCancel(wxCommandEvent&)
{
	...
	if (wxMessageBoxEx(_("Really cancel current operation?"), _T("TabFTP"), wxYES_NO | wxICON_QUESTION) == wxYES) {
...
void CMainFrame::OnClose(wxCloseEvent &event)
{
	if (!m_bQuit) {
		...
		if (event.CanVeto()) {
			...
			if (m_pQueueView && m_pQueueView->IsActive()) {
				...
				dlg.SetTitle(_("Close TabFTP"));

				dlg.AddText(_("File transfers still in progress."));
				dlg.AddText(_("Do you really want to close TabFTP?"));
			...
			if (edit_handler_) {
					...
					edit_handler_->GetFileCount(CEditHandler::none, CEditHandler::upload_and_remove_failed))
				{
					...
					dlg.AddText(_("If you close TabFTP, your changes will be lost."));
					dlg.AddText(_("Do you really want to close TabFTP?"));
	...
	if (m_pContextControl) {
		CContextControl::_context_controls* controls = m_pContextControl->GetCurrentControls();
		if (m_pContextControl->GetGlobalLocalListView()) {
			m_pContextControl->GetGlobalLocalListView()->SaveColumnSettings(OPTION_LOCALFILELIST_COLUMN_WIDTHS, OPTION_LOCALFILELIST_COLUMN_SHOWN, OPTION_LOCALFILELIST_COLUMN_ORDER);
		}
		if (controls && controls->pRemoteListView) {
...
void CMainFrame::OnMenuEditSettings(wxCommandEvent&)
{
	...
	if (oldLang != newLang) {
		wxMessageBoxEx(_("TabFTP needs to be restarted for the language change to take effect."), _("Language changed"), wxICON_INFORMATION, this);
	}
...
void CMainFrame::OnToggleDirectoryTreeView(wxCommandEvent& event)
{
	...
	CSplitterWindowEx* splitter;
	if (local) {
		splitter = m_pContextControl->GetGlobalLocalSplitter();
	}
	else {
		CContextControl::_context_controls* controls = m_pContextControl->GetCurrentControls();
		if (!controls) {
		...
		splitter = controls->pRemoteSplitter;
	}
...
void CMainFrame::ShowDirectoryTree(bool local, bool show)
{
	...
	m_pContextControl->UpdateAllLayouts();
}
...
bool CMainFrame::ConnectToSite(Site & data, Bookmark const& bookmark, CState* pState)
{
	...
	if (pState->IsRemoteConnected() || !pState->IsRemoteIdle()) {
		...
		if (action < 2) {
			...
			main->Add(new wxStaticText(&dlg, nullID, _("TabFTP is already connected to a server.")));
...
void CMainFrame::OnChar(wxKeyEvent& event)
{
	...
	CContextControl::_context_controls* controls = m_pContextControl->GetCurrentControls();
	if (controls) {
		windowOrder.push_back(m_pContextControl->GetGlobalLocalViewHeader());
...
bool CMainFrame::RestoreSplitterPositions()
{
	...
	// top_pos bottom_height view_pos view_height_width queuelog_splitter
	...
	if (m_pContextControl) {
		...
		for (int i = 0; i < m_pContextControl->GetTabCount(); ++i) {
			...
			controls->SetSplitterPositions(std::make_tuple(pos, values[3]));
		}
	}

	double pos = static_cast<double>(values[4]) / 1000000000;
...
void CMainFrame::OnToolbarComparison(wxCommandEvent&)
{
	...
	if (m_pContextControl) {
		CContextControl::_context_controls* controls = m_pContextControl->GetCurrentControls();
		if (controls && controls->pRemoteListView) {
			CComparableListing* leftListing{};

			if (auto* leftTabs = m_pContextControl->GetLeftTabControl(); leftTabs) {
				int const leftSel = leftTabs->GetSelection();
				if (leftSel != wxNOT_FOUND && leftSel < static_cast<int>(leftTabs->GetPageCount())) {
					wxWindow* leftPage = leftTabs->GetPage(leftSel);
					if (auto* local = m_pContextControl->GetLocalControlsFromPage(leftPage); local && local->pListView) {
						leftListing = local->pListView;
					}
					else if (auto* remoteLeft = m_pContextControl->GetRemoteControlsFromPage(leftPage); remoteLeft && remoteLeft->pRemoteListView) {
						leftListing = remoteLeft->pRemoteListView;
					}
				}
			}

			if (leftListing && leftListing != controls->pRemoteListView) {
				pComparisonManager->SetListings(leftListing, controls->pRemoteListView);
			}
		}
	}
	...
	if (!options_.get_int(OPTION_FILEPANE_LAYOUT)) {
		...
		CListSearchPanel* leftSearchPanel{};
		CSplitterWindowEx* leftSplitter{};

		if (auto* leftTabs = m_pContextControl->GetLeftTabControl(); leftTabs) {
			int const leftSel = leftTabs->GetSelection();
			if (leftSel != wxNOT_FOUND && leftSel < static_cast<int>(leftTabs->GetPageCount())) {
				wxWindow* leftPage = leftTabs->GetPage(leftSel);
				if (auto* local = m_pContextControl->GetLocalControlsFromPage(leftPage); local) {
					leftSearchPanel = local->pListSearchPanel;
					leftSplitter = local->pSplitter;
				}
				else if (auto* remoteLeft = m_pContextControl->GetRemoteControlsFromPage(leftPage); remoteLeft) {
					leftSearchPanel = remoteLeft->pRemoteListSearchPanel;
					leftSplitter = remoteLeft->pRemoteSplitter;
				}
			}
		}

		if ((leftSearchPanel && leftSearchPanel->IsShown()) || (controls->pRemoteListSearchPanel && controls->pRemoteListSearchPanel->IsShown())) {
			...
			if (leftSearchPanel && leftSearchPanel->IsShown()) {
				leftSearchPanel->Close();
			}
		...
		if (leftSplitter && controls->pRemoteSplitter) {
			if ((leftSplitter->IsSplit() && !controls->pRemoteSplitter->IsSplit()) ||
				(!leftSplitter->IsSplit() && controls->pRemoteSplitter->IsSplit()))
			{
			...
			int pos = (leftSplitter->GetSashPosition() + controls->pRemoteSplitter->GetSashPosition()) / 2;
			leftSplitter->SetSashPosition(pos);
...
WXLRESULT CMainFrame::MSWWindowProc(WXUINT nMsg, WXWPARAM wParam, WXLPARAM lParam)
{
	if (nMsg == WM_DEVICECHANGE) {
		...
		// Use global local tree view since it's now shared across all tabs
		CLocalTreeView* pLocalTreeView = m_pContextControl->GetGlobalLocalTreeView();
		if (pLocalTreeView) {
			pLocalTreeView->OnDevicechange(wParam, lParam);
		}
		return 0;
	}
...
#ifndef __WXMSW__
		wxIcon icon = GetCustomAppIcon(32);
		if (icon.IsOk()) {
			installed = m_taskBarIcon->SetIcon(icon, GetTitle());
		}
		else {
			installed = m_taskBarIcon->SetIcon(CThemeProvider::GetIcon(_T("ART_TABFTP")), GetTitle());
		}
#else
		installed = m_taskBarIcon->SetIcon(CThemeProvider::GetIcon(_T("ART_TABFTP")), GetTitle());
#endif
