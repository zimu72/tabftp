...
std::string CInitializer::GetDefaultsXmlFile()
{
	...
	std::string unadjSettingsDir = GetUnadjustedSettingsDir();
	if (!unadjSettingsDir.empty()) {
		file = unadjSettingsDir + "fzdefaults.xml";
	...
	file = CheckPathForDefaults(fzdatadir, 0, "fzdefaults.xml");
	if (!file.empty()) {
		return file;
	}
...
std::string CInitializer::GetSettingFromFile(std::string file, const std::string& name)
{
	...
	auto main = xmldoc.child("TabFTP3");
	if (!main) {
		main = xmldoc.child("FileZilla3");
	}
	if (!main) {
		return "";
	}
...
{
	std::string cfg = TryDirectory("XDG_CONFIG_HOME", "tabftp/", true);
	if (cfg.empty() ) {
		cfg = TryDirectory("HOME", ".config/tabftp/", true);
	}
	if (cfg.empty() ) {
		cfg = TryDirectory("HOME", ".tabftp/", true);
	}
	if (cfg.empty() ) {
		cfg = TryDirectory("XDG_CONFIG_HOME", "tabftp/", false);
	}
	if (cfg.empty() ) {
		cfg = TryDirectory("HOME", ".config/tabftp/", false);
	}
	if (cfg.empty() ) {
		cfg = TryDirectory("HOME", ".tabftp/", false);
	}

	if (cfg.empty() ) {
		cfg = TryDirectory("XDG_CONFIG_HOME", "filezilla/", true);
	}
	if (cfg.empty() ) {
...
#ifdef __WXDEBUG__
	printf("Reading locale option from %stabftp.xml\n", dir.c_str());
#endif
	std::string locale = GetSettingFromFile(dir + "tabftp.xml", "Language Code");
	if (locale.empty()) {
		locale = GetSettingFromFile(dir + "filezilla.xml", "Language Code");
	}
