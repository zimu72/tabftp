			...
			case colSize:
				{
					...
					if (size >= 0) {
						return SizeFormatter(options_).Format(size, SizeFormatPurpose::normal);
					}
					else {
						return _T("?");
					}
...
CQueue::CQueue(wxWindow* parent, CMainFrame *pMainFrame, CAsyncRequestQueue *pAsyncRequestQueue, cert_store & certStore)
{
	...
	// Defer queue loading to after window is shown for faster perceived startup
	// Use CallAfter to load queue in next event loop iteration
	CallAfter([this]() {
		m_pQueueView->LoadQueue();
	});

#if FZ_WINDOWS
	auto evt = wxEVT_COMMAND_AUINOTEBOOK_TAB_RIGHT_UP;
#else
	auto evt = wxEVT_COMMAND_AUINOTEBOOK_TAB_RIGHT_DOWN;
#endif

	Bind(evt, [this](wxAuiNotebookEvent& ev) {
