...
bool COptionsPageUpdateCheck::CreateControls(wxWindow* parent)
{
	...
	auto [box, inner] = lay.createStatBox(main, _("TabFTP updates"), 1);
	inner->AddGrowableRow(6);

	inner->Add(new wxStaticText(box, nullID, _("Check for TabFTP &updates automatically:")));
	...
	inner->Add(new wxStaticText(box, nullID, _("To check for updates, information such as the version of TabFTP, your operating system and your CPU architecture needs to be submitted. Only necessary data is submitted. It is collected in anonymized, aggregate form only.")));
	inner->Add(new wxHyperlinkCtrl(box, nullID, _("Privacy policy"), L"https://github.com/zimu72/tabftp"));
