...
bool COptionsPageConnectionActive::CreateControls(wxWindow* parent)
{
...

	{
		...
		impl_->limit_ = new wxCheckBox(box, nullID, _("&Limit local ports used by TabFTP"));
		impl_->limit_->Bind(wxEVT_CHECKBOX, onChange);
		inner->Add(impl_->limit_);

		inner->Add(new wxStaticText(box, nullID, _("By default TabFTP uses any available local port to establish transfers in active mode. If you want to limit TabFTP to use only a small range of ports, please enter the port range below.")), 0, wxLEFT, lay.indent);
...

	{
		...
		inner->Add(new wxStaticText(box, nullID, _("In order to use active mode, TabFTP needs to know your external IP address.")));
		...
		inner->Add(new wxStaticText(box, nullID, _("Default: http://ip.tabftp.org/ip.php")), 0, wxLEFT, lay.indent);
