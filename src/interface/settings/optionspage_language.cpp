...
bool COptionsPageLanguage::CreateControls(wxWindow* parent)
{
	...
	inner->Add(new wxStaticText(box, nullID, _("If you change the language, you need to restart TabFTP.")));
...
void COptionsPageLanguage::GetLocales()
{
	...
	for (bool found = dir.GetFirst(&locale); found; found = dir.GetNext(&locale)) {
		if (!wxFileName::FileExists(localesDir.GetPath() + locale + _T("/tabftp.mo")) &&
			!wxFileName::FileExists(localesDir.GetPath() + locale + _T("/LC_MESSAGES/tabftp.mo")) &&
			!wxFileName::FileExists(localesDir.GetPath() + locale + _T("/filezilla.mo")) &&
			!wxFileName::FileExists(localesDir.GetPath() + locale + _T("/LC_MESSAGES/filezilla.mo")))
		{
			continue;
		}
