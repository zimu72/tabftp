...
static unsigned int register_interface_options()
{
	...
	static int const value = register_options({
		...
		{ "Show Tree Local", false, option_flags::normal },
		...
		{ "Icon theme", L"modern_1", option_flags::normal },
...
CLocalPath COptions::GetCacheDirectory()
{
	...
	if (!dir.empty()) {
		...
		if (!ret.empty()) {
			ret.AddSegment(L"TabFTP");
		}
#else
		std::wstring cfg = TryDirectory(GetEnv("XDG_CACHE_HOME"), L"tabftp/", false);
		if (cfg.empty()) {
			cfg = TryDirectory(wxGetHomeDir(), L".cache/tabftp/", false);
		}
		ret.SetPath(cfg);
#endif
	}
