...
public:
	...
	virtual wxDragResult OnData(wxCoord x, wxCoord y, wxDragResult def)
	{
		...
		if (obj->GetSite().server != m_pRemoteListView->m_state.GetSite().server) {
			// Server-to-Server Proxy Transfer
			std::wstring subdir;
			int flags{};
			int hit = m_pRemoteListView->HitTest(wxPoint(x, y), flags, 0);
			if (hit != -1 && (flags & wxLIST_HITTEST_ONITEM)) {
			...
			CServerPath targetPath = m_pRemoteListView->m_pDirectoryListing->path;
			if (subdir == L"..") {
				if (targetPath.HasParent()) targetPath = targetPath.GetParent();
			} else if (!subdir.empty()) {
				targetPath.AddSegment(subdir);
			}
			...
			if (!hasDirs) {
				for (auto const& info : files) {
					m_pRemoteListView->m_pQueue->QueueProxyTransfer(
						obj->GetSite(), obj->GetServerPath(), info.name,
						m_pRemoteListView->m_state.GetSite(), targetPath
					);
				}
				return wxDragCopy;
			}
			...
			CDragDropManager* pDragDropManager = CDragDropManager::Get();
			CStateEventHandler* pSourceHandler = nullptr;
			if (pDragDropManager && pDragDropManager->pDragSource) {
				pSourceHandler = dynamic_cast<CStateEventHandler*>(const_cast<wxWindow*>(pDragDropManager->pDragSource));
			}

			CState* pSourceState{};
			if (pSourceHandler && pSourceHandler->m_state.GetSite() && pSourceHandler->m_state.GetSite().server == obj->GetSite().server) {
				pSourceState = &pSourceHandler->m_state;
			}
			else {
				CContextManager* pManager = CContextManager::Get();
				if (pManager) {
					auto const states = pManager->GetAllStates();
					if (states) {
						for (auto* pState : *states) {
							if (pState && pState->IsRemoteConnected() && pState->GetSite() && pState->GetSite().server == obj->GetSite().server) {
								pSourceState = pState;
								break;
							}
						}
			...
			if (!pSourceState) {
				wxMessageBoxEx(_("Could not locate source view for recursive transfer."));
				return wxDragNone;
			}

			CServerPath sourcePath = obj->GetServerPath();
			recursion_root root(sourcePath, false);

#ifdef __WXMSW__
			std::wstring dummyRoot = L"C:\\ProxyTransfer\\";
#else
			std::wstring dummyRoot = L"/ProxyTransfer/";
#endif
			CLocalPath localRoot(dummyRoot);

			for (auto const& info : files) {
				if (info.dir) {
					CLocalPath subDirLocal = localRoot;
					subDirLocal.AddSegment(info.name);
					root.add_dir_to_visit(sourcePath, info.name, subDirLocal, false, true);
				}
				else {
					m_pRemoteListView->m_pQueue->QueueProxyTransfer(
						obj->GetSite(), obj->GetServerPath(), info.name,
			...
			if (!root.empty()) {
				pSourceState->GetRemoteRecursiveOperation()->SetProxyTarget(m_pRemoteListView->m_state.GetSite(), targetPath, dummyRoot);
				pSourceState->GetRemoteRecursiveOperation()->AddRecursionRoot(std::move(root));
				pSourceState->GetRemoteRecursiveOperation()->StartRecursiveOperation(recursive_operation::recursive_proxy_transfer, ActiveFilters());
			}
...
void CRemoteListView::OnItemActivated(wxListEvent &event)
{
	...
	if (item) {
		...
		if (entry.is_dir()) {
			...
			// Get local path from global local view if available
			CLocalPath local_path;
			CContextControl* pContextControl = m_state.GetMainFrame().GetContextControl();
			if (pContextControl) {
				CLocalListView* pGlobalLocalListView = pContextControl->GetGlobalLocalListView();
				if (pGlobalLocalListView) {
					local_path = pGlobalLocalListView->GetDir();
				}
			}
			// Fallback to state's local dir if global local view not available
			if (local_path.empty()) {
				local_path = m_state.GetLocalDir();
			}
...
void CRemoteListView::OnMenuDelete(wxCommandEvent&)
{
	...
	if (selected_link) {
		...
		main->Add(new wxStaticText(&dlg, nullID, _("If the symlink points to a directory, TabFTP can either delete the symbolic link or remove the contents of the linked directory.")));
