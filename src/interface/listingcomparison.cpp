...
CComparableListing::~CComparableListing()
{
	if (m_pComparisonManager) {
		m_pComparisonManager->RemoveListing(this);
	}

	auto* contextManager = CContextManager::Get();
	if (contextManager) {
		auto const* states = contextManager->GetAllStates();
		if (states) {
			for (auto* state : *states) {
				if (state) {
					state->GetComparisonManager()->RemoveListing(this);
				}
			}
		}
	}
...
void CComparisonManager::SetListings(CComparableListing* pLeft, CComparableListing* pRight)
{
	...
	if (m_pLeft) {
		m_pLeft->SetOther(0);
		if (m_pLeft->m_pComparisonManager == this) m_pLeft->m_pComparisonManager = 0;
	}
	if (m_pRight) {
		m_pRight->SetOther(0);
		if (m_pRight->m_pComparisonManager == this) m_pRight->m_pComparisonManager = 0;
	}
...
void CComparisonManager::RemoveListing(CComparableListing* pListing)
{
	if (!pListing) {
		return;
	}
	...
	if (m_pLeft == pListing) {
		...
		pListing->SetOther(0);
		if (pListing->m_pComparisonManager == this) {
			pListing->m_pComparisonManager = 0;
		}
		m_pLeft = 0;
	}
	if (m_pRight == pListing) {
		if (m_pLeft) {
			m_pLeft->SetOther(0);
		}
		pListing->SetOther(0);
