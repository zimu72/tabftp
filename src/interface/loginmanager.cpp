...
bool CLoginManager::query_unprotect_site(Site & site)
{
	...
	main->AddSpacer(0);
	auto* remember = new wxCheckBox(&pwdDlg, -1, _("&Remember master password until TabFTP is closed"));
...
bool CLoginManager::query_credentials(Site & site, std::wstring const& challenge, bool otp, bool canRemember)
{
	...
	if (canRemember && needs_pass) {
		main->AddSpacer(0);
		remember = new wxCheckBox(&pwdDlg, -1, _("&Remember password until TabFTP is closed"));
		remember->SetValue(true);
		main->Add(remember);
	}
