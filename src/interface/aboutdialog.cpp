...
{
	wxDialogEx::Create(parent, -1, _("About TabFTP"));
	...
	top->Add(CThemeProvider::Get()->createStaticBitmap(this, L"ART_TABFTP", iconSizeLarge), 0, wxALL, lay.border);

	auto topRight = lay.createFlex(1);
	top->Add(topRight);

	std::wstring version = L"TabFTP " + GetFileZillaVersion();
	if (CBuildInfo::GetBuildType() == L"nightly") {
		version += L"-nightly";
	}
	topRight->Add(new wxStaticText(this, -1, version));
	topRight->Add(new wxStaticText(this, -1, L"TabFTP is based on FileZilla by Tim Kosse."));
	...
	homepage->Add(new wxHyperlinkCtrl(this, -1, L"https://github.com/zimu72/tabftp", L"https://github.com/zimu72/tabftp"), lay.valign);
...
{
	wxString text = _T("TabFTP Client\n");
	text += _T("----------------\n\n");
