...
unsigned int register_common_options()
{
	...
	static int const value = register_options({ { "Config Location", L"", option_flags::predefined_only | option_flags::platform },
		...
		{ "Remote recursive operation list refresh",  false, option_flags::normal }
	});
...
bool XmlOptions::Load(std::wstring & error)
{
	...
	std::wstring const tabftp = dir.GetPath() + L"tabftp.xml";
	std::wstring const filezilla = dir.GetPath() + L"filezilla.xml";

	bool const hasTabftp = fz::local_filesys::get_size(fz::to_native(tabftp)) > 0;
	bool const hasFilezilla = fz::local_filesys::get_size(fz::to_native(filezilla)) > 0;

	std::wstring path = tabftp;
	if (!hasTabftp && hasFilezilla) {
		path = filezilla;
	}

	xmlFile_ = std::make_unique<CXmlFile>(path);
	if (!xmlFile_->Load()) {
		error = xmlFile_->GetError();
	}
	else {
		if (!hasTabftp && hasFilezilla) {
			xmlFile_->SetFileName(tabftp);
			if (!xmlFile_->Save(true)) {
				xmlFile_->SetFileName(filezilla);
			}
		}
