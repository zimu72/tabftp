...
{
	fz::uri uri("https://update.tabftp.org/update.php");
	fz::query_string qs;
	...
	auto key = fz::regkey(HKEY_CURRENT_USER, L"Software\\TabFTP Client", true, fz::regkey::regview_32);
	if (!key) {
		key.open(HKEY_LOCAL_MACHINE, L"Software\\TabFTP Client", true, fz::regkey::regview_32);
	}
