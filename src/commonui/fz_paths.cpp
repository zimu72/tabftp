...
#else
	std::wstring cfg = TryDirectory(GetEnv("XDG_CONFIG_HOME"), L"tabftp/", true);
	if (cfg.empty()) {
		cfg = TryDirectory(GetEnv("HOME"), L".config/tabftp/", true);
	}
	if (cfg.empty()) {
		cfg = TryDirectory(GetEnv("HOME"), L".tabftp/", true);
	}
	if (cfg.empty()) {
		cfg = TryDirectory(GetEnv("XDG_CONFIG_HOME"), L"tabftp/", false);
	}
	if (cfg.empty()) {
		cfg = TryDirectory(GetEnv("HOME"), L".config/tabftp/", false);
	}
	if (cfg.empty()) {
		cfg = TryDirectory(GetEnv("HOME"), L".tabftp/", false);
	}

	if (cfg.empty()) {
		cfg = TryDirectory(GetEnv("XDG_CONFIG_HOME"), L"filezilla/", true);
	}
	if (cfg.empty()) {
...
{
#ifdef FZ_WINDOWS
	std::wstring const path = GetOwnExecutable();
...
CLocalPath GetFZDataDir(std::vector<std::wstring> const& fileToFind, std::wstring const& prefixSub, bool searchSelfDir)
{
	...
	if (!selfDir.empty()) {
		...
		if (!prefixSub.empty() && selfDir.size() > 5 && fz::ends_with(selfDir, std::wstring(L_DIR_SEP L"lib" L_DIR_SEP))) {
			std::wstring path = selfDir.substr(0, selfDir.size() - 4) + prefixSub + L_DIR_SEP;
			if (testPath(path)) {
				return ret;
			}
		}
		if (!prefixSub.empty() && selfDir.size() > 7 && fz::ends_with(selfDir, std::wstring(L_DIR_SEP L"lib64" L_DIR_SEP))) {
			std::wstring path = selfDir.substr(0, selfDir.size() - 6) + prefixSub + L_DIR_SEP;
			if (testPath(path)) {
				return ret;
			}
		}
	...
	if (!prefixSub.empty()) {
		...
		for (auto const& segment : segments) {
			auto cur = CLocalPath(segment).GetPath();
	...
	if (!prefixSub.empty()) {
		...
		path = std::wstring(L_DIR_SEP) + prefixSub + L_DIR_SEP;
		if (testPath(path)) {
			return ret;
		}

		path = std::wstring(L_DIR_SEP L"usr" L_DIR_SEP) + prefixSub + L_DIR_SEP;
		if (testPath(path)) {
			return ret;
		}

		path = std::wstring(L_DIR_SEP L"usr" L_DIR_SEP L"local" L_DIR_SEP) + prefixSub + L_DIR_SEP;
		if (testPath(path)) {
			return ret;
		}

		path = std::wstring(L_DIR_SEP L"opt" L_DIR_SEP) + prefixSub + L_DIR_SEP;
		if (testPath(path)) {
			return ret;
		}

		path = std::wstring(L_DIR_SEP L"opt" L_DIR_SEP L"tabftp" L_DIR_SEP) + prefixSub + L_DIR_SEP;
		if (testPath(path)) {
			return ret;
		}
	}
...
CLocalPath GetDefaultsDir()
{
	static CLocalPath path = [] {
		...
		if (path.empty() || !FileExists(path.GetPath() + L"fzdefaults.xml")) {
			if (FileExists(L"/etc/tabftp/fzdefaults.xml")) {
				path.SetPath(L"/etc/tabftp");
			}
			else if (FileExists(L"/etc/filezilla/fzdefaults.xml")) {
...
#endif
		if (path.empty()) {
			path = GetFZDataDir({ L"fzdefaults.xml" }, L"share/tabftp");
			if (path.empty()) {
