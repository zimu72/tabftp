...
public:
	...
	bool SetupPassiveTransfer(std::wstring const& host, unsigned short port);
...
protected:
	...
	struct passive_mode_data {
		std::string host_;
		uint16_t port_{};
		uint16_t attempts_;
	};
	std::optional<passive_mode_data> passive_mode_data_;
