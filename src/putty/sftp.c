...
static int fxp_got_status(struct sftp_packet *pktin)
{
    ...
    if (pktin->type != SSH_FXP_STATUS) {
        ...
        if (get_err(pktin)) {
            fxp_error_message = dupstr("malformed FXP_STATUS packet");
            fxp_errtype = -1;
        } else {
            char * error_desc = NULL;
            ptrlen ed = get_string(pktin);
            if (!get_err(pktin)) {
                error_desc = mkstr(ed);
            }
            
            if (error_desc && *error_desc && fxp_errtype >= SSH_FX_FAILURE) {
                ...
                if (fxp_errtype < 0 || fxp_errtype >= lenof(messages))
                    fxp_error_message = dupstr("received unknown error code");
                else
