    ...
    bool reset_sequence_number)
{
    ...
    if (cipher) {
        ...
        fzprintf(sftpCipherClientToServer, ssh_cipher_alg(s->out.cipher)->text_name);
    ...
    if (mac) {
        ...
        fzprintf(sftpMacClientToServer, etm_mode ? "%s (in ETM mode)" : "%s", ssh2_mac_text_name(s->out.mac));
    ...
    bool reset_sequence_number)
{
    ...
    if (cipher) {
        ...
        fzprintf(sftpCipherServerToClient, ssh_cipher_alg(s->in.cipher)->text_name);
    ...
    if (mac) {
        ...
        fzprintf(sftpMacServerToClient, etm_mode ? "%s (in ETM mode)" : "%s", ssh2_mac_text_name(s->in.mac));
