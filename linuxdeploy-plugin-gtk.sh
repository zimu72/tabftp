...
if [ -f "$APPDIR/${gdk_pixbuf_cache_file/$LD_GTK_LIBRARY_PATH//usr/lib}" ]; then
    sed -E 's|"(/usr/lib(64)?[^"]*/gdk-pixbuf-2.0/[^"]*/loaders)/|"@GDK_PIXBUF_MODULEDIR@/|g' \
        "$APPDIR/${gdk_pixbuf_cache_file/$LD_GTK_LIBRARY_PATH//usr/lib}" > "$APPDIR/${gdk_pixbuf_cache_file/$LD_GTK_LIBRARY_PATH//usr/lib}.in"
fi
cat >> "$HOOKFILE" <<EOF
gdk_pixbuf_moduledir="\$APPDIR/${gdk_pixbuf_moduledir/$LD_GTK_LIBRARY_PATH//usr/lib}"
gdk_pixbuf_template="\$APPDIR/${gdk_pixbuf_cache_file/$LD_GTK_LIBRARY_PATH//usr/lib}.in"
gdk_pixbuf_cache_base="\${XDG_CACHE_HOME:-\$HOME/.cache}"
if [ -n "\${gdk_pixbuf_cache_base:-}" ] && mkdir -p "\$gdk_pixbuf_cache_base/tabftp" 2>/dev/null && [ -w "\$gdk_pixbuf_cache_base/tabftp" ]; then
    gdk_pixbuf_cache_dir="\$gdk_pixbuf_cache_base/tabftp"
else
    gdk_pixbuf_cache_dir="\$(mktemp -d /tmp/tabftp-gdkpixbuf.XXXXXX)"
fi
